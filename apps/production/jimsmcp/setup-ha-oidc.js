...
class AuthentikOIDCClient extends AuthentikClient {
     ...
     */
    async updateApplicationProvider(appPk, providerId) {
...
async function main() {
    ...
    try {
        ...
        const existingProvider = providers.find((p) => p.name === 'Home Assistant OIDC' || p.name === 'Home Assistant OAuth2');
        if (existingProvider) {
        ...
        let haApp = apps.find((app) => app.slug === 'ha' || app.name === 'Home Assistant');
        if (haApp) {
