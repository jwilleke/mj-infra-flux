...
class AuthentikOIDCClient extends AuthentikClient {
   ...
   */
  async updateApplicationProvider(appSlug: string, providerId: number) {
   ...
   */
  async addRedirectUri(providerId: number, redirectUri: string) {
    ...
    const uriExists = existingUris.some((uri: any) =>
      (typeof uri === 'string' ? uri : uri.url) === redirectUri
    );
...
async function main() {
  ...
  try {
    ...
    const oauth2Provider = providers.find((p: any) =>
      p.name === 'Home Assistant OAuth2' || p.name === 'Home Assistant OIDC'
    );
    ...
    let amdwikiApp = apps.find((app: any) =>
      app.slug === 'amdwiki' || app.name === 'AMDWiki'
    );
